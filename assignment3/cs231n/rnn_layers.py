...
def rnn_backward(dh, cache):
	...
	db = np.zeros_like(b)
	dh_step = np.zeros_like(h0)
	t = x.shape[1]
	for i in xrange(t-1,0,-1):
		dx[:,i,:], dh_step, dWx_step, dWh_step, db_step = rnn_step_backward(dh[:,i,:] + dh_step,(h[:,i,:], x[:,i,:], h[:,i-1,:], Wx, Wh, b))
		dWx = dWx + dWx_step
		dWh = dWh + dWh_step
		db = db + db_step
	dx[:,0,:], dh_step, dWx_step, dWh_step, db_step = rnn_step_backward(dh[:,0,:] + dh_step,(h[:,0,:], x[:,0,:], h0, Wx, Wh, b))
	dWx = dWx + dWx_step
	dWh = dWh + dWh_step
	db = db + db_step
	dh0 = dh_step
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
	...
	#############################################################################
	h = prev_h.shape[1]
	ifog = np.dot(x,Wx) + np.dot(prev_h,Wh) + b[None,:]
	i = sigmoid(ifog[:,:1*h])
	f = sigmoid(ifog[:,1*h:2*h])
...
def lstm_forward(x, h0, Wx, Wh, b):
	...
	h = np.zeros((x.shape[0],t,h0.shape[1]))
	c = np.zeros_like(h)
	h[:,0,:],c[:,0,:],_ = lstm_step_forward(x[:,0,:], h0, np.zeros_like(h0), Wx, Wh, b)
	cache.append(_)
	for i in xrange(1,t):
		h[:,i,:],c[:,i,:],_ = lstm_step_forward(x[:,i,:], h[:,i-1,:], c[:,i-1,:], Wx, Wh, b)
		cache.append(_)
	pass
	##############################################################################
	#                               END OF YOUR CODE                             #
...
def lstm_backward(dh, cache):
	...
	#############################################################################
	_,_,_,_,_,_,x,_,_,Wx,Wh = cache[0]
	N,D = x.shape
	t = dh.shape[1]
	dx = np.zeros((N,t,D))
	dWx = np.zeros_like(Wx)
	...
	dh_step = np.zeros_like(_)
	dc_step = np.zeros_like(_)
	for i in xrange(t-1,0,-1):
		dx[:,i,:], dh_step, dc_step, dWx_step, dWh_step, db_step = lstm_step_backward(dh[:,i,:] + dh_step, dc_step, cache[i])
		dWx = dWx + dWx_step
		dWh = dWh + dWh_step
		db = db + db_step
	dx[:,0,:], dh_step, dc_step,dWx_step, dWh_step, db_step = lstm_step_backward(dh[:,0,:] + dh_step, dc_step, cache[0])
	dWx = dWx + dWx_step
	dWh = dWh + dWh_step
	db = db + db_step
	dh0 = dh_step
